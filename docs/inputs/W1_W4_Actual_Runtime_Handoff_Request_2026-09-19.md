...
상태: **W4 후보 검토 회신 수령 완료 / 실제 runtime 인계 대기 / AWS 연결 미착수**
...
## 4. W1이 A–F 수령 뒤 수행할 작업

W1은 A–F가 모두 충족되고 D의 identity가 안전 채널로 확인된 뒤에만 다음을 수행한다.

1. W4 전용 isolated SQS main queue와 DLQ를 생성한다.
2. W4 send-only Role에 main queue `SendMessage`만 허용하는 queue policy를 적용한다.
3. W1 Worker Role에 해당 main queue의 receive/delete/change-visibility와 필요한 attribute 조회만
   부여한다. W4에는 receive/delete/W1 DB 권한을 부여하지 않는다.
4. `W4_QUESTION_CORE_DECISION_QUEUE_URL`, `W4_QUESTION_CORE_DECISION_DLQ_URL`,
   `W4_QUESTION_CORE_DECISION_EXPECTED_PRODUCER=w4`,
   `W4_QUESTION_CORE_DECISION_EXPECTED_SENDER_ID`를 별도 worker env 파일에 주입한다.
5. immutable W1/W4 image pin으로 W1 preflight를 통과한 뒤 `w4-question-core` profile을 시작한다.
6. W4/W1/W2가 공동 CT-12를 실행하고, terminal receipt·중복·stale·취소/삭제·명시적 retry/W2
   command 경계를 실제 PostgreSQL/SQS에서 기록한다.
