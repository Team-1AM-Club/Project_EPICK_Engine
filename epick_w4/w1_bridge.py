...
def validate_binding(binding, context, owner, run):
    if (any(not isinstance(getattr(binding, key), UUID) for key in
            ("owner_user_id", "run_id", "project_id", "question_id", "question_version_id", "snapshot_id", "lease_token"))
    ...
    if (context["project"] != {"owner_id": str(owner), "project_id": str(binding.project_id)}
...
def publication_for(binding, result):
    ...
    if (result["request_id"] != str(binding.run_id) or result["project_id"] != str(binding.project_id)
    ...
    result_version = "w4-" + content_hash({"run_id": str(binding.run_id),
        ...
        "result": result})
    mapping = {(e.episode_id, e.version): str(e.episode_version_id) for e in binding.episode_versions}
    statuses = {"DIRECT_MATCH": "DIRECT_MATCH", "PARTIAL_MATCH": "PARTIAL_RELEVANCE",
    ...
    for number, candidate in enumerate(result["candidates"], 1):
        ...
        candidates.append({"episode_version_id": mapping[key], "match_status": statuses[candidate["status"]],
            "short_reason": reasons or "원문 근거를 추가로 확인해야 합니다.",
            "strength_summary": reasons or None,
            "limitation_summary": questions or "합성 검증 결과이며 문항·정답·순위 정책은 팀 검토 전입니다.",
...
class W1ExecutionAdapter:
    ...
    def __init__(self, *, store, extraction_factory, judgment_factory, c01_consumer):
        self.store, self.extraction_factory, self.judgment_factory = store, extraction_factory, judgment_factory
    ...
    def execute(self, *, owner_user_id: UUID, run_id: UUID) -> None:
        ...
        try:
            ...
            context, request = validate_binding(binding, self.store.load_context(binding), owner_user_id, run_id)
            result = execute_service(request, user_id=str(owner_user_id), backend=backend,
                extraction_factory=self.extraction_factory, judgment_factory=self.judgment_factory,
                c01_consumer=self.c01_consumer, c01_split=True)
